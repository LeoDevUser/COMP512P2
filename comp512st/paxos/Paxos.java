...
class PaxosListener implements Runnable, Serializable {
	Paxos paxos;
	int curQuorumPropose = 0;
	int curQuorumAccept = 0;
	Map<Integer, Integer> acceptAckCounts = new HashMap<>();
	...
	void handlePaxosMessage(PaxosMessage m) throws IllegalArgumentException, InterruptedException {
		...
		switch(m.type) {
			...
			case PROMISE -> {
				if(m.ballotID == paxos.ballotID.get()) curQuorumPropose++;
				...
				//do we need to handle case where we get promise for higher ballotID than current???
			}
			case ACCEPT -> {//only leader should send these
			...
			case ACCEPTACK -> {
				...
				if(m.ballotID == paxos.ballotID.get()) {
					int count = acceptAckCounts.getOrDefault(TO_val.seq, 0) + 1;
					acceptAckCounts.put(TO_val.seq, count);
					if (count == paxos.quorum) {//quorum reached
						msg = new PaxosMessage(paxos.ballotID.get(), PaxosType.CONFIRM, m.val, paxos.myProcess);
						paxos.gcl.broadcastMsg(msg);
						acceptAckCounts.remove(TO_val.seq);
					}
				}
			}
			case CONFIRM -> {
			...
			case DENY -> {
				paxos.ballotID.set(paxos.ballotID.get()+1);//try again with higherBallotID
				if(paxos.leader == null) {//elect new leader
					...
					//check for errors if this is reached
				} else {//stable leader so send request directly to it
